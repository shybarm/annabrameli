  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/20 hover:bg-muted/40 transition-colors border border-border/30">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {page.notes && (
          <p className="text-xs text-amber-600 dark:text-amber-400">{page.notes}</p>
        )}
        <div className="flex items-center gap-2 flex-wrap">
...
function ClusterCard({ cluster }: { cluster: TopicCluster }) {
  ...
  return (
    <Card className="border-border/50 overflow-hidden">
      <CardHeader
        className="cursor-pointer hover:bg-muted/20 transition-colors pb-3"
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${v.color} ${v.bg}`}>
                {v.label}
              </span>
            </div>
    ...
    <Card className="border-border/50">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          AI Coverage Depth — עומק כיסוי נושאי
        </CardTitle>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="mt-4 p-3 rounded-lg border border-destructive/30 bg-destructive/5">
          ...
          <ul className="text-xs text-muted-foreground space-y-0.5">
            {sorted
              .filter(c => c.coverageVerdict === 'minimal' || c.coverageVerdict === 'thin')
              .map(c => (
    ...
    <Card className="border-border/50">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          Answer Intent Map — מפת כוונות תשובה
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          קיבוץ כל הדפים לפי סוג הכוונה שהם עונים עליה. חושף פערים בסוגי תשובות שהאתר לא מכסה.
        </p>
      </CardHeader>
            ...
            <div key={intent} className="p-3 rounded-lg border border-border/50 bg-card">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  {totalMissing > 0 && (
                    <Badge variant="destructive" className="text-[10px]">{totalMissing} חסרים</Badge>
                  )}
                </div>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20">
        <h3 className="text-sm font-bold text-foreground mb-1">
          Sprint 4 — מפת אשכולות נושאיים ופערי תוכן
        </h3>
        <p className="text-xs text-muted-foreground leading-relaxed">
          מיפוי מלא של נושאי האתר לאשכולות, זיהוי פערים בתוכן, ובדיקת עומק כיסוי עבור מנועי AI.
          אשכול מקיף = סיכוי גבוה יותר שמערכת AI תבחר את האתר כמקור סמכותי.
        </p>
      </div>
      ...
      <Tabs defaultValue="clusters" className="w-full">
        <TabsList className="w-full h-auto gap-1 bg-muted/30 p-1 rounded-xl flex-wrap">
          ...
          <TabsTrigger value="coverage" className="text-xs rounded-lg">AI Coverage Depth</TabsTrigger>
          <TabsTrigger value="intents" className="text-xs rounded-lg">Answer Intent Map</TabsTrigger>
        </TabsList>
