...
`;
    ...
    const systemPrompt = `אתה רופא מומחה שמסכם מסמכים רפואיים. כתוב סיכום קצר וממוקד בלבד.

כללים:
- הסיכום חייב להיות קצר - עד 5-6 נקודות מרכזיות בלבד
- התמקד רק בממצאים חשובים ביותר
- ציין ערכים חריגים במספרים (אם יש)
- אל תפרט - תמצת!
- כתוב בסגנון נקודות קצרות`;

    const userPrompt = `${patientName ? `מטופל: ${patientName}\n\n` : ''}מסמכים לסיכום:\n\n${allDocumentsContent}

כתוב סיכום קצר של 5-6 נקודות עיקריות בלבד. התמקד בממצאים הכי חשובים.`;
